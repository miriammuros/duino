...
var Board = function (options) {
  ...
  // We want to set the serial port of the port in case we have more than one
  this.detect(this.serialPort, function (err, serial) {
    if (err) {
...
Board.prototype.detect = function (serialPort, callback) {
  ...
  if (serialPort) {
    try {
      temp = new serial.SerialPort('/dev/' + serialPort, {
...
Board.prototype.dimmerWrite = function (pin, val) {
  pin = this.normalizePin(pin);
  val = this.normalizeVal(val);
  this.log('info', 'dimmerWrite to pin ' + pin + ': ' + val.green);
  this.write('05' + pin + val);
}
